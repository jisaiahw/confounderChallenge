...
ggplot(patients_df, aes(x = patient, y = post_surgical_score, 
  ...
  geom_hline(yintercept = mean_dreamy, 
             linetype = "dashed", linewidth = 0.8, color = "#4E79A7") +
  geom_hline(yintercept = mean_duck, 
             linetype = "dashed", linewidth = 0.8, color = "#E15759") +
  annotate("text", x = max(patients_df$patient) * 0.95, 
           y = mean_dreamy + 0.15, 
  ...
  annotate("text", x = max(patients_df$patient) * 0.95, 
           ...
           color = "#E15759", size = 4, fontface = "bold") +
  scale_color_manual(
    name = "Doctor",
    values = c("Doc Dreamy" = "#4E79A7", "Doc Duck" = "#E15759"),
    labels = c("Doc Dreamy" = "Doc Dreamy (○)", "Doc Duck" = "Doc Duck (△)")
  ) +
  scale_shape_manual(
    name = "Doctor",
    values = c("Doc Dreamy" = 19, "Doc Duck" = 17),
    labels = c("Doc Dreamy" = "Doc Dreamy (○)", "Doc Duck" = "Doc Duck (△)")
  ) +
  labs(
...
ggplot(patients_randomized_df, aes(x = patient, y = post_surgical_score, 
                                   color = doctor_name, shape = doctor_name)) +
  geom_point(size = 2.5, alpha = 0.7) +
  geom_hline(yintercept = mean_dreamy_rand, 
             linetype = "dashed", linewidth = 0.8, color = "#4E79A7") +
  geom_hline(yintercept = mean_duck_rand, 
             linetype = "dashed", linewidth = 0.8, color = "#E15759") +
  annotate("text", x = max(patients_randomized_df$patient) * 0.95, 
           y = mean_dreamy_rand + 0.15, 
           label = paste0("○ ", round(mean_dreamy_rand, 2)), 
           color = "#4E79A7", size = 4, fontface = "bold") +
  annotate("text", x = max(patients_randomized_df$patient) * 0.95, 
           y = mean_duck_rand + 0.15, 
           label = paste0("△ ", round(mean_duck_rand, 2)), 
           color = "#E15759", size = 4, fontface = "bold") +
  scale_color_manual(
    name = "Doctor",
    values = c("Doc Dreamy" = "#4E79A7", "Doc Duck" = "#E15759"),
  ...
  labs(
    title = "Post-Surgical Symptom Score by Patient and Doctor (Randomized Assignment)",
    x = "Patient Number",
    y = "Post-Surgical Symptom Score (Lower is Better)",
    color = "Doctor",
    shape = "Doctor"
...
ggplot(patients_df, aes(x = severity, y = post_surgical_score, 
  ...
  geom_point(size = 2.5, alpha = 0.7) +
  geom_hline(yintercept = mean_dreamy_sev, 
             linetype = "dashed", linewidth = 0.8, color = "#4E79A7") +
  geom_hline(yintercept = mean_duck_sev, 
             linetype = "dashed", linewidth = 0.8, color = "#E15759") +
  annotate("text", x = max(patients_df$severity) * 0.9, 
           y = mean_dreamy_sev + 0.15, 
           label = paste0("○ ", round(mean_dreamy_sev, 2)), 
           color = "#4E79A7", size = 4, fontface = "bold") +
  annotate("text", x = max(patients_df$severity) * 0.9, 
           y = mean_duck_sev + 0.15, 
           label = paste0("△ ", round(mean_duck_sev, 2)), 
           color = "#E15759", size = 4, fontface = "bold") +
  scale_color_manual(
    name = "Doctor",
    values = c("Doc Dreamy" = "#4E79A7", "Doc Duck" = "#E15759"),
...
## Conclusion: Learning from Confounding
...
The real villain here is patient severity. It operates like a puppeteer behind the curtain, making Doc Dreamy look good by sending him easier cases while sending Doc Duck the harder ones. When you don't randomize, you can't tell the difference between "Doc Dreamy is a better surgeon" and "Doc Dreamy got lucky with his patient assignment." On any graph or in any headline, look for the common-cause confounder that is unaccounted for.
